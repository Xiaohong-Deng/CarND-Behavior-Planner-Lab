...
class Road(object):
    ...
    def populate_traffic(self):
        for lane_num in range(self.num_lanes):
            ...
            # coordinates in the longitude direction
            iterator = iter(range(self.visible_length))
            for s in iterator:
                # decide if we want to populate the coordinate with a car
    ...
    def advance(self):
        predictions = {}
        # Generate predictions/trajectories represented by a sequence of vehicles
        for v_id, v in self.vehicles.items():
            # predict other cars' future trajectories
            if v_id != self.ego_key:
                # horizon = 2 by default
                # preds is a sequence of Vehicles with different s, v, a, lane num, representing
                # the trajectory
                # length of preds is n = horizon, time span required to be covered by predictions
        ...
        for v_id, v in self.vehicles.items():
            if v_id == self.ego_key:
                # choose the lowest cost next possible state based on other cars'
                # future trajectories
                trajectory = v.choose_next_state(predictions)
                # set the ego car's state with the end state of the trajectory
                v.realize_next_state(trajectory)
            else:
                # compute s in the next second for cars that are not ego
                # and set that s as v.s
                v.increment()
    ...
    def add_ego(self, lane_num, s, config_data):
        ...
        ego.state = "KL"
        # ego_key is set at the top of the file
