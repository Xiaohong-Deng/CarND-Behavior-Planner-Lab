...
class Vehicle(object):
    ...
    def choose_next_state(self, predictions):
        ...
        costs = []
        for state in states:
            # generate ego car's trajectory in the next 2 time steps based on the state
            trajectory = self.generate_trajectory(state, predictions)
            if trajectory:
                # if trajectory is possible compute its cost
    ...
    def successor_states(self):
        ...
        if self.state == "KL":
            ...
            states = ["KL"]
            # so the lanes are from 0 to the largest number, right to left
            # self.lanes_available - 1 is the leftmost
    ...
    def generate_trajectory(self, state, predictions):
        """
        Given a possible next state and the future trajectory of the surrounding cars
        generate a trajectory to realize the next state.
        """
        if state == "CS":
    ...
    def get_kinematics(self, predictions, lane):
        ...
        """
        max_velocity_accel_limit = self.max_acceleration + self.v  # v + at where t=1?
    ...
    def keep_lane_trajectory(self, predictions):
        # trajectory covers 2 time steps (including the current time step), keep lane so lane doesn't change
        trajectory = [Vehicle(self.lane, self.s, self.v, self.a, self.state)]
    ...
    def generate_predictions(self, horizon=2):
        ...
        Inputs:
        ----
        horizon: number of seconds in the future over which you
                 predict the vehicle's position

        Outputs:
        ----
        predictions: [Vehicle(self.lane, s, v, 0), ...]. Predictions considers
                      only the vehicle in one lane. In other words, after
                      and before any lane shifts
        """
        predictions = []
        for i in range(horizon):
            # s = 0 if i = 0, otherwise s = s + v*t + 0.5 * a * t**2
            s = self.position_at(i)
            v = 0
            if i < horizon - 1:
                # v between i + 1 and i second
    ...
    def configure(self, road_data):
        ...
        """
        self.target_speed = road_data['speed_limit']  # road_data is config_data in simulation
